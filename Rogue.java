...
public class Rogue extends Brawler implements Character {
	...
	public boolean canAttack(Brawler victim) {
		...
		if (this.getFloor() != victim.getFloor()) ambush = 1;
		if (victim instanceof Serpent) isSerpent = 1;
		int defense = (victim.getLevel() + victim.getShieldBonus() + victim.getClericDefenseBonus() - isSerpent);
		int attack = (this.getLevel() + this.getSwordBonus() + ambush);
		...
		else if (!this.isHandsFull() && (victim instanceof Merchant || victim instanceof Treasure)) answer = true;
		return answer;
	}
